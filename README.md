...
- **Server-only API key**: stored in `GROQ_API_KEY`, never shipped to the browser.
...
| LLM | Groq `llama-3.1-8b-instant` (default, configurable) | Fast responses with strong instruction-following. |
...
persona-chatbot/
├── app/
│   ├── api/chat/route.ts      # Server route — calls Groq
...
Requirements: Node 18.17+ and a Groq API key.
...
- Optional: set `GROQ_MODEL` if you want a different model.
